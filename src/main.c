...
void *angle1_left_func(void *args) {
  int *result = malloc(sizeof(int));
  cicle:
    //printf("Thread angle1_left_func working\n");
    pthread_cond_wait(&distance_cond, &angle1_mutex);
    pthread_mutex_lock(&vertical_angle1_mutex);
    if (vertical_angle1 == true) {
      pthread_mutex_unlock(&vertical_angle1_mutex);
...
void *angle2_right_func(void *argss) {
  int *result = malloc(sizeof(int));
  cicle:
    //printf("Thread angle2_right_func working\n");
    pthread_cond_wait(&distance_cond, &angle2_mutex);
    pthread_mutex_lock(&vertical_angle2_mutex);
    if (vertical_angle2 == true) {
      pthread_mutex_unlock(&vertical_angle2_mutex);
      *result = 0;
      return (void *)result;
    }
...
void *angle2_left_func(void *args) {
  int *result = malloc(sizeof(int));
  cicle:
    //printf("Thread angle2_left_func working\n");
    pthread_cond_wait(&distance_cond, &angle2_mutex);
    pthread_mutex_lock(&vertical_angle2_mutex);
    if (vertical_angle2 == true) {
      pthread_mutex_unlock(&vertical_angle2_mutex);
    ...
    if (angle2 == 0.0) {
      ...
      *result = 0;
      ...
      angle2 = angle2 - 0.5;
      printf(
          "Plane 2 | ✋ Tilting backwards | ✅ Backward engine ON | Angle %2.2f\n",
          angle2);
    } else if (angle2 < 0.0) { // negative
      pthread_mutex_unlock(&angle2_mutex);
      *result = 0;
